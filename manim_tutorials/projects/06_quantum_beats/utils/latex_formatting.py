...
class QuantumLatexFormatter:
    ...
    def master_equation(dissipator: bool = True) -> str:
        ...
        """
        base = rf'\frac{{d\hat{{\rho}}}}{{dt}} = -\frac{{i}}{{\hbar}}[\hat{{H}}, \hat{{\rho}}]'
...
QUANTUM_BEAT_EXPRESSIONS = {
    ...
    'beat_frequency': rf'\Delta = \omega_2 - \omega_1 = \frac{E_2 - E_1}{\hbar}',
    
    'coherence_decay': rf'\rho_{12}(t) = \rho_{12}(0) e^{-i\Delta t - \Gamma_{12} t}',
    
    'decoherence_rate': rf'\Gamma_{12} = \frac{\gamma_1 + \gamma_2}{2}',
    
    'beat_visibility': rf'V = \frac{2\sqrt{p_1 p_2}}{p_1 + p_2}',
    
    'rotating_frame': rf'\tilde{H} = \hat{H} - \hbar\omega_L \sum_i |i\rangle\langle i|',
    
    'dipole_interaction': (
        rf'\hat{H}_{int} = -\vec{\mu} \cdot \vec{E} = -\sum_{{i,j}} \mu_{{ij}} E(t) '
        rf'(|i\rangle\langle j| + |j\rangle\langle i|)'
    ),
    
    'population_dynamics': (
        rf'\frac{dp_i}{dt} = -\gamma_i p_i + \sum_{{j>i}} \gamma_{{ji}} p_j'
    ),
    
    'coherence_dynamics': (
        rf'\frac{d\rho_{{ij}}}{dt} = -i\omega_{{ij}} \rho_{{ij}} - \Gamma_{{ij}} \rho_{{ij}}'
    )
}
